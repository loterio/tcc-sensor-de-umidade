...
void loop() {
  ...
  Serial.println("Leitura Recebida: " + String(leituraRecebida));
    ...
    if (dadosAtualizados) {
      ...
      if (idSensor == 0) {
        ...
        while ((millis() - tempoInicial) <= tempoTentandoEnviarId) {
          unsigned long tempoEsperandoResposta = tempoTentandoEnviarId/5;
          enviaDadoLora(novoIdSensorJson);
          String respostaDeEnvio = recebeDadoLora(tempoEsperandoResposta);
...
void enviaDadoLora (String dado) {
  Serial.println("Dado Enviado: " + dado);
  lora.print(dado);
}
...
String recebeDadoLora(unsigned long tempoEspera) {  
  ...
  while ((millis() - tempoInicial) <= tempoEspera) {
    if (lora.available() > 1) {
      ...
      Serial.println("Tempo Inicial: " + String(tempoInicial));
      Serial.println("Tempo Espera: " + String(tempoEspera));
